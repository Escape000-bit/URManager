...
set_security_patch() {
    ...
    if [ -f "/data/adb/modules/playintegrityfix/pif.json" ]; then
        ...
        echo "[DEBUG] No pif.json file found."
    fi

    # Extract security patch
    security_patch=$(grep '"SECURITY_PATCH"' "$PIF" | sed 's/.*: "//; s/".*//')
    echo "[DEBUG] SECURITY_PATCH from JSON: $security_patch"

    # Fallback to getprop
    if [ -z "$security_patch" ]; then
        security_patch=$(getprop ro.build.version.security_patch)
        echo "[DEBUG] Fallback SECURITY_PATCH from getprop: $security_patch"
    fi

    # Format
    formatted_security_patch=$(echo "$security_patch" | sed 's/-//g')
    echo "[DEBUG] Formatted: $formatted_security_patch"

    security_patch_after_1y=$(echo "$formatted_security_patch + 10000" | bc)
    echo "[DEBUG] Valid until: $security_patch_after_1y"
    ...
    if [ -n "$formatted_security_patch" ] && [ "$TODAY" -lt "$security_patch_after_1y" ]; then
        echo "[DEBUG] Security patch is valid."

        TS_version=$(grep "versionCode=" "/data/adb/modules/tricky_store/module.prop" | cut -d'=' -f2)
        echo "[DEBUG] Tricky Store version: $TS_version"

        if [ "$TS_version" -lt 158 ]; then
            echo "[DEBUG] Setting properties directly (resetprop)..."
            resetprop ro.vendor.build.security_patch "$security_patch"
            resetprop ro.build.version.security_patch "$security_patch"
            echo "[DEBUG] Properties set to: $security_patch"
        else
            echo "[DEBUG] Writing patch to file for newer TS version..."
            ...
            echo "[DEBUG] File written: $SECURITY_PATCH_FILE"
        fi
    else
        echo "[DEBUG] No valid patch found or older than 1 year. Not set."
        echo "not set"
    fi

    echo "[DEBUG] set_security_patch finished."
}
