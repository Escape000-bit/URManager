...
ui_print "Berechne SHA256 für: $BOOTIMAGE"
if [ -f "$BOOTIMAGE" ]; then
  BOOT_CHECKSUM=$(sha256sum "$BOOTIMAGE" | awk '{ print $1 }')
  ui_print "SHA256 (Original Boot Image): $BOOT_CHECKSUM"
else
  ui_print "Fehler: Datei $BOOTIMAGE nicht gefunden!"
fi

ui_print "Berechne SHA256 für: new-boot.img"
if [ -f "./new-boot.img" ]; then
  PATCHED_CHECKSUM=$(sha256sum "./new-boot.img" | awk '{ print $1 }')
  ui_print "SHA256 (Patched Boot Image): $PATCHED_CHECKSUM"
else
  ui_print "Fehler: Datei new-boot.img nicht gefunden!"
fi
